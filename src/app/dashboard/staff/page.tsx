...
import { useAuthStore, useStaffStore, type Staff } from '@/lib/store'
import {
    Card,
    CardContent,
    CardHeader,
...
export default function StaffPage() {
    ...
    const handleSubmitNewStaff = async () => {
        // 表单验证
        if (!newStaff.name || !newStaff.phone || !newStaff.email || !newStaff.position) {
    ...
    const handleUpdateStaff = async () => {
        ...
        // 表单验证
        if (!editingStaff.name || !editingStaff.phone || !editingStaff.email || !editingStaff.position) {
            toast.error('请填写所有必填项')
            return
        }
