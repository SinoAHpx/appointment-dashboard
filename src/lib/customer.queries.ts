	...
	try {
       ...
       RETURNING id, name, phone, email, address, company, createdAt`,
		...
		if (
			...
			console.error(
				`Error adding customer: Phone or Email might already exist.`,
			);
		} else {
...
export const updateCustomer = (
	...
	try {
		...
		if (fields.length === 0) {
			const currentQuery = db.query<Customer, [number]>(
       ...
       RETURNING id, name, phone, email, address, company, createdAt`,
		...
		if (
			...
			console.error(
				`Error updating customer ${id}: Phone or Email might already exist.`,
			);
		} else {
