...
export const addVehicle = (data: NewVehicleData): Vehicle | null => {
    try {
        ...
        const insertQuery = db.query<Vehicle, [string, string | null, Vehicle['status']]>(
        ...
        if (error instanceof Error && error.message.includes('UNIQUE constraint failed: vehicles.plateNumber')) {
 ...
 */
export const updateVehicle = (id: number, data: UpdateVehicleData): Vehicle | null => {
    try {
        const db = getDb();
        const fields = Object.keys(data) as (keyof UpdateVehicleData)[];
        if (fields.length === 0) {
            const currentQuery = db.query<Vehicle, [number]>("SELECT * FROM vehicles WHERE id = ?");
        ...
        if (data.plateNumber && error instanceof Error && error.message.includes('UNIQUE constraint failed: vehicles.plateNumber')) {
            console.error(`Error updating vehicle ${id}: Plate number already exists.`);
        } else {
...
export const deleteVehicle = (id: number): boolean => {
    try {
        ...
        db.query("UPDATE appointments SET vehicleId = NULL WHERE vehicleId = ?").run(id);
