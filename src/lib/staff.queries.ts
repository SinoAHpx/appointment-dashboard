 ...
 */
export const addStaff = (name: string, status: Staff['status'] = 'active'): Staff | null => {
    try {
        const db = getDb();
 ...
 */
export const updateStaff = (id: number, data: Partial<Pick<Staff, 'name' | 'status'>>): Staff | null => {
    try {
        const db = getDb();
        let updateClause = "";
        ...
        if (values.length === 0) {
            ...
            const currentStaffQuery = db.query<Staff, [number]>("SELECT id, name, status, createdAt FROM staff WHERE id = ?");
...
export const deleteStaff = (id: number): boolean => {
    try {
        ...
        db.query("UPDATE appointments SET staffId = NULL WHERE staffId = ?").run(id);
